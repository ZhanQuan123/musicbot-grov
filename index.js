...
client.on("ready", () => {
    ...
    client.user.setActivity({ type: "LISTENING", name: `to -help`, status: 'dnd' })
})
...
client.distube
    ...
    .on("addSong", (message, queue, song) => message.channel.send(
      new Discord.MessageEmbed()
            .setColor('#66e12d')
            .setDescription(`Queued[${song.name} [${song.user}]](${song.url})`)
    ))
    .on("playList", (message, queue, playlist, song) => message.channel.send(
        new Discord.MessageEmbed()
            .setColor('#66e12d')
            .setDescription(`Queued [${playlist.title} playlist ${playlist.total_items}](${song.url}) songs [${song.user}.]`)
    ))
